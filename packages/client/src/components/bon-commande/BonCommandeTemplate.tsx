...
export const buildBonCommandeTemplate = (bc: BonCommande): string => {
  const recipientLines: string[] = [];
  if (bc.supplier) {
    if (bc.supplier.name) recipientLines.push(bc.supplier.name);
    if (bc.supplier.company) recipientLines.push(bc.supplier.company);
    if (bc.supplier.address) recipientLines.push(bc.supplier.address);
    if (bc.supplier.email) recipientLines.push("Email : " + bc.supplier.email);
    if (bc.supplier.phone) recipientLines.push("Tél : " + bc.supplier.phone);
    if (bc.supplier.taxId)
      recipientLines.push("Matricule : " + bc.supplier.taxId);
  }

  const lineItemsHtml = bc.lines
    .map(
      (line) => `
  ...
  const statusLabel = statusLabels[bc.status] ?? bc.status.toUpperCase();
  ...
  return `
    ...
    <head>
        <meta charset="UTF-8">
        <title>Bon de Commande ${bc.number}</title>
        <style>
      ...
      <div class="page-container">
        ...
        <table style="width: 100%; margin-bottom: 30px; border-collapse: collapse;">
          <tr>
            <td style="width: 40%; vertical-align: middle;">
              <div style="font-size: 20px; font-weight: 700; color: #111827;">BON DE COMMANDE</div>
            </td>
            <td style="width: 60%; text-align: right; vertical-align: middle;">
              <h1 class="heavy" style="font-size: 38px; text-transform: uppercase; margin: 0; padding: 0; line-height: 1;">BC</h1>
              <div style="margin-top: 6px;">
                <span style="display: inline-block; padding: 3px 10px; border-radius: 4px; font-size: 11px; font-weight: 700;
                  background: ${bc.status === "confirme" ? "#d1fae5" : bc.status === "annule" ? "#fee2e2" : "#f3f4f6"};
                  color: ${bc.status === "confirme" ? "#065f46" : bc.status === "annule" ? "#991b1b" : "#374151"};">
                  ${statusLabel}
                </span>
              </div>
            </td>
        ...
        <table style="width: 100%; margin-bottom: 20px; font-size: 13px; border-collapse: collapse;">
          <tr>
            <td style="width: 50%; vertical-align: top;">
              <div><span class="bold">DATE :</span> ${new Date(bc.createdAt).toLocaleDateString("fr-FR")}</div>
              <div><span class="bold">VALIDE JUSQU'AU :</span> ${new Date(bc.validUntil).toLocaleDateString("fr-FR")}</div>
            </td>
            <td style="width: 50%; text-align: right; vertical-align: top;">
              <div class="bold" style="font-size: 15px;">BC N° : ${bc.number}</div>
            </td>
          </tr>
        </table>
        ...
        <table style="width: 100%; margin-bottom: 40px; font-size: 12px; border-collapse: collapse;">
          <tr>
            <td style="width: 50%; vertical-align: top; padding-right: 20px;">
              <!-- left intentionally empty or can show issuer if needed -->
            </td>
          ...
          <tr>
            <td style="width: 55%; vertical-align: top; padding-right: 20px; font-size: 12px;">
              ...
              <div style="margin-top: 10px; font-size: 10px; color: #6b7280; line-height: 1.4;">
                Ce bon de commande est valable jusqu'au ${new Date(bc.validUntil).toLocaleDateString("fr-FR")}.<br/>
            ...
            <td style="width: 45%; vertical-align: top; text-align: right; padding-left: 20px;">
              <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
                <tr>
                  ...
                  <td style="padding: 6px 0; text-align: right; white-space: nowrap;">${formatAmount(bc.amountHT)}</td>
                </tr>
                <tr>
                  <td class="bold" style="padding: 6px 0; text-align: right; padding-right: 15px;">TVA (${bc.tvaRate}%) :</td>
                  <td style="padding: 6px 0; text-align: right; white-space: nowrap;">${formatAmount(bc.amountTVA)}</td>
                </tr>
                <tr style="border-top: 2px solid #333;">
                  <td class="bold" style="padding: 12px 0 6px 0; text-align: right; padding-right: 15px; font-size: 15px;">TOTAL TTC :</td>
                  <td class="heavy" style="padding: 12px 0 6px 0; text-align: right; font-size: 17px; white-space: nowrap;">${formatAmount(bc.amountTTC)}</td>
                </tr>
              </table>
            </td>
          </tr>
        ...
        ${
          bc.notes
            ...
            <p style="margin: 0;"><strong style="color: #111827;">Note :</strong> ${bc.notes}</p>
          </div>`
            : ""
        }
