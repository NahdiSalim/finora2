...
const createLine = (): BonCommandeLine => ({
...
const defaultValues: BonCommandeFormValues = {
  number: "",
  status: "brouillon",
  tvaRate: 19,
  validUntil: "",
...
export default function BonCommandeModal({
  ...
  const [create, { isLoading: isCreating }] = useCreateBonCommandeMutation();
  const [update, { isLoading: isUpdating }] = useUpdateBonCommandeMutation();
  ...
  } = useForm<BonCommandeFormValues>({ mode: "onChange", defaultValues });

  // Pre-fill form in edit mode — must be after useForm so reset is defined
  useEffect(() => {
    if (open && initialData) {
      reset({
  ...
  const onSubmit = async (data: BonCommandeFormValues) => {
    try {
        ...
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            {isEditMode
              ...
              : "Créez un bon de commande professionnel"}
          </Typography>
        </Box>
